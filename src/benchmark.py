...
def heatmap_visualize(data, fname, args):
    bg_cmap = 'gray'
    hm_cmap = 'Blues'
    hm_alpha = 0.7 if args.overlay else 1.
    if args.metric == 'all':
        cb_shrink = 0.6
        ...
        possible_dims = [(i, int(np.ceil(tot_num_to_plot / i))) for i in range(4, int(np.ceil(np.sqrt(tot_num_to_plot) + 1)))]
        dims = possible_dims[np.argmin([
        ...
        for metric in list_of_metrics:
            if metric not in single_metrics:
                ...
                metric_plot = raveled_axs[i].imshow(data['metrics'][metric], alpha=hm_alpha, cmap=hm_cmap)
                ...
                if data['T'] is not None:
                    ...
                    metric_plot = raveled_axs[i].imshow(data['metrics'][metric][0], alpha=hm_alpha, cmap=hm_cmap)
                    raveled_axs[i].set_title(metric + ' on X')
                    fig.colorbar(metric_plot, ax=raveled_axs[i], shrink=cb_shrink)
                    i += 1
                    if args.overlay:
                        raveled_axs[i].imshow(data['T'], cmap=bg_cmap)
                    metric_plot = raveled_axs[i].imshow(data['metrics'][metric][1], alpha=hm_alpha, cmap=hm_cmap)
        ...
        if args.metric not in single_metrics:
            ...
            axs[0, 1].imshow(data['metrics'][0], alpha=hm_alpha, cmap=hm_cmap, clim=(cmin, cmax))
            if args.overlay:
                axs[1, 1].imshow(data['T'], cmap=bg_cmap)
            plt2 = axs[1, 1].imshow(data['metrics'][1], alpha=hm_alpha, cmap=hm_cmap, clim=(cmin, cmax))
